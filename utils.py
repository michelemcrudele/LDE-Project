...
def SIR_net_adaptive_NeutralState(G, NET, beta, mu, r, pro, pol, p_sym, initial_infecteds, initial_no_vax, rewiring=True, seed=123):
    """
    G = physical network
...
def SIRS_net_adaptive(G, NET, beta, mu, gamma, r, pro, pol, p_sym, nmax, initial_infecteds, rewiring=True, seed=123, message=True):
    """
    G: physical network
    NET: information network,
    beta: infection rate,
    ...
    while t < nmax:
        ...
        for i in nx.nodes(G):
            ...
            if G.nodes[i]['inf_status'] == 'R' or G.nodes[i]['inf_status'] == 'V':
